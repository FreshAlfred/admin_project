<template>
  <el-form label-width="auto">
    ...
    <el-form-item label="SPU销售属性">
      <el-select v-model="saleAttrNameAndValuename" :placeholder="unSelectSaleAttr.length
      ...
      <el-button :disabled="!saleAttrNameAndValuename.length" style="margin-left: 10px" icon="Plus" type="primary"
        size="default" @click="addSaleAttr">
        添加属性值
      </el-button>
        ...
        <el-table-column label="销售属性值">
          <template #="{ row, $index }">
            <el-tag @close="row.spuSaleAttrValueList.splice(index, 1)" v-for="(item, index) in row.spuSaleAttrValueList"
              :key="item.id" class="mx-1" style="margin: 0 5px" closable :type="row.type">
              {{ item.saleAttrValueName }}
            </el-tag>
            <el-input @blur="toLook(row)" v-model="row.saleAttrValue" v-if="row.flag === true" placeholder="请输入销售属性值"
      ...
      <el-form-item>
        <el-button type="primary" size="default" :disabled="saleAttr.length > 0 ? false : true" @click="save">
          保存
        </el-button>
